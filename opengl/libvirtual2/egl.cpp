...
struct egl_surface_t
{
   ...
   /*IDC_GLES2 start*/
    virtual     EGLBoolean	SetSurfaceparam(FILE* fd, int32_t surfaceEnumerator, int32_t width, int32_t height, uint32_t physicalAddr, uint32_t virtualAddr, int32_t pid, uint32_t pixelFormat, uint32_t pixelType, uint32_t stride);
    virtual     EGLBoolean  SetpixelFormat(EGLint pixformat);
    virtual     EGLBoolean  convertPixelParam(int pixelFormat,int* pixformat,int* pixtype);
    virtual     EGLint      getPixelFormat() const;
    /*IDC_GLES2 end*/
protected:
    GGLSurface              depth;
};
...
/*IDC_GLES2 start*/
EGLBoolean egl_surface_t::SetSurfaceparam(FILE* fd, int32_t surfaceEnumerator, int32_t width, int32_t height, uint32_t physicalAddr, uint32_t virtualAddr, int32_t pid, uint32_t pixelFormat, uint32_t pixelType, uint32_t stride)
{
...
/*IDC_GLES2 start*/
    ...
    virtual     EGLBoolean  SetpixelFormat(EGLint pixformat);
    virtual     EGLBoolean  convertPixelParam(int pixelFormat,int* pixformat,int* pixtype);
    virtual     EGLint      getPixelFormat() const { return pixelFormat; }
/*IDC_GLES2 end*/
...
private:
 ...
 /*IDC_GLES2 start*/
    status_t getSurfacePhysicalAddr(android_native_buffer_t* buf, void** paddr);
 /*IDC_GLES2 end*/
...
EGLBoolean egl_window_surface_v2_t::connect() 
{
 ...
 /*IDC_GLES2 start*/
/*1. Get the virtual buffer address
  2. Call Gralloc and get the Physicall address
  3. pack param into structure and pass it to the host*/
   LOGV(" EGL  connectSwapbuffer Getphyaddr of the buffer...  %p (%ux%u) %x",
                    buffer, buffer->width, buffer->height,&phyaddr);
  if(getSurfacePhysicalAddr(buffer,&phyaddr)!=NO_ERROR){
   return setError(EGL_BAD_ACCESS, EGL_FALSE);
	}
 /*IDC_GLES2 end*/
...
/*IDC_GLES2 start*/
status_t egl_window_surface_v2_t::getSurfacePhysicalAddr(android_native_buffer_t* buf, void** paddr)
{
    int err = NO_ERROR;
    if (sw_gralloc_handle_t::validate(buf->handle) < 0) {
        err = module->gles2emulator_getPhysAddr(module, buf->handle,paddr);
     LOGV("Selva:: Egl getphyaddr  %x",*paddr);
     }
     return err;
}
...
/*IDC_GLES2 start*/
EGLBoolean egl_window_surface_v2_t::convertPixelParam(int pixelFormat,int* pixformat,int* pixtype )
{
    LOGV("selva::convertPixelParam \n");
 switch(pixelFormat)
 {
   case GGL_PIXEL_FORMAT_RGB_565:
        *pixformat = EGL_RGB;
        *pixtype = EGL_UNSIGNED_SHORT_5_6_5;
        LOGV("selva::convertPixelParam EGL_RGB\n");
        break;
    case GGL_PIXEL_FORMAT_RGBX_8888:
        *pixformat = EGL_RGBA;
        *pixtype = EGL_UNSIGNED_INT_8_8_8_8;
        break;
        ...
        EGLint attribute, EGLint *value)
{
    ...
    LOGV("GLES2 sets EGL_BAD_ATTR in get config attrib...");
        ...
        NativeWindowType window, const EGLint *attrib_list)
{
    ...
    EGLint surfaceType;
     LOGV("GLES2 createWindowSurface get config attrib");
                            ...
                            EGLint *num_config)
{
    ...
    if (ggl_unlikely(num_config==0)) {
     ...
     LOGV("GLES2 Enter eglChooseConfig 2");
    ...
    *num_config = n;
    LOGV("GLES2 Enter eglChooseConfig return %d",*num_config);
     return EGL_TRUE;
}
                            ...
                            EGLint attribute, EGLint *value)
{
    ...
    switch (attribute) {
        case EGL_CONFIG_ID:
            LOGV("GLES2 eglQuerySurface get config attrib");
        ...
        case EGL_SWAP_BEHAVIOR:
            *value = surface->getSwapBehavior();
            break;
        default:
            LOGV("GLES2 error EGL_BAD_ATTRIBUTE %d",attribute);
...
/*IDC_GLES2 end*/
            ...
            if (d) {
                ...
                d->ctx = ctx;
/*IDC_GLES2 start*/
//		LOGV("selva::call bind draw surface from Make");
//                d->SetSurfaceparam((ogles_context_t*)d->ctx);
/*IDC_GLES2 end*/
...
void libvirtual_init(egl_context_t* c)
	{
		...
		if (c->theVirtualDeviceFileDescriptor)
		{
			...
			c->theVirtualDeviceDescriptor = fileno (c->theVirtualDeviceFileDescriptor);
		...
		LOGV ("[libEGL->VirtualGL] Opening device %s ", theVirtualDeviceIOCTLFilename);
		c->theVirtualDeviceIOCTLFileDescriptor = fopen (theVirtualDeviceIOCTLFilename, "r+");
